...
class RecommendedPlot:
    ...
    label: str
    reason: str
    key: str = ""              # stable identifier for build_plot_inputs dispatch
...
def recommend_plots(matrix_spec: MatrixSpec,
    ...
    def add(key, plot_type, label, reason, *, transforms=None, inputs=None,
            status=READY, sci=None, vis=None, shape="matrix"):
        recs.append(RecommendedPlot(plot_type, label, reason, key=key,
    ...
    add("heatmap", "heatmap_clustered_matrix", "Heatmap", "Visualize the whole value matrix.",
        status=matrix_status,
        vis=(["Large matrix — defaults to top-variable features for legibility."] if big else []))
    add("heatmap_zscore", "heatmap_clustered_matrix", "Clustered heatmap (row z-score)",
        "Cluster features/samples on standardized values.",
        transforms=["row_zscore"], status=matrix_status)
    add("clustering", "hierarchical_clustering", "Hierarchical clustering",
        "Cluster and reorder the matrix.", status=matrix_status)
    add("dendrogram", "hierarchical_dendrogram", "Dendrogram", "Sample/feature clustering tree.",
        status=matrix_status)
    add("sample_correlation_heatmap", "heatmap_clustered_matrix", "Sample correlation heatmap",
        "Sample-by-sample correlation structure / QC.",
        transforms=["sample_correlation"], status=matrix_status)
    add("pca", "pca_scatter_from_matrix", "PCA scatter", "Sample structure in low dimensions.",
        transforms=["pca"], status=matrix_status,
        sci=(["Need >= 3 samples for a meaningful PCA."] if n_val and n_val < 3 else []))
    if not big:
        add("feature_correlation_heatmap", "heatmap_clustered_matrix", "Feature correlation heatmap",
            "Feature-feature correlation (manageable feature count).",
            transforms=["feature_correlation"], status=matrix_status)
    add("top_variable_heatmap", "heatmap_clustered_matrix", "Top variable-feature heatmap",
    ...
    for key, pt, label in (
            ("box_by_group", "boxplot_or_violin_with_points", "Selected-feature box/violin by group"),
            ("dot_by_group", "dot_strip_plot", "Selected-feature dot/strip by group"),
            ("raincloud_by_group", "raincloud_plot", "Selected-feature raincloud by group"),
            ("bar_by_group", "barplot_with_error_bar", "Selected-feature bar (mean +/- error) by group")):
        add(key, pt, label, "Compare chosen features across groups.",
            transforms=["wide_to_long"], inputs=["select feature(s)"],
            status=group_status, shape="long")
    add("pca_by_group", "pca_scatter_from_matrix", "PCA colored by group",
        "Sample structure colored by condition.", transforms=["pca"], status=group_status)
    ...
    for key, pt, label, reason in (
        ("volcano", "volcano_plot", "Volcano", "Effect vs significance from a differential summary."),
        ("ma", "ma_plot", "MA plot", "Effect vs mean abundance from a differential summary."),
        ("ranked_effect", "waterfall_plot", "Ranked effect (waterfall)",
         "Top features ranked by effect size."),
    ):
        add(key, pt, label, reason,
...
def recommend_from_differential_table(has_average_abundance: bool = False,
    ...
    recs.append(RecommendedPlot("volcano_plot", "Volcano",
                "Effect vs significance from your precomputed table.", key="volcano",
                required_data_shape="differential_table"))
    if has_average_abundance:
        recs.append(RecommendedPlot("ma_plot", "MA plot",
                    "Effect vs average abundance.", key="ma",
                    required_data_shape="differential_table"))
    recs.append(RecommendedPlot("waterfall_plot", "Ranked effect (waterfall)",
                "Top features ranked by effect.", key="ranked_effect",
                required_data_shape="differential_table"))
    if has_matrix:
        recs.append(RecommendedPlot("heatmap_clustered_matrix", "Top-feature heatmap",
                    "Heatmap of the top differential features from the matrix.", key="top_feature_heatmap",
