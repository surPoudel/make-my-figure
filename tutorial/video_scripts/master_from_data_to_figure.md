...
| 3:45-4:30 | 3. Options, figure | Show individual observations off / on; Jitter width 0.10 then 0.35; Point size 60; Point fill open then filled with a dark edge; Sample-size labels below | "Every observation is drawn on the summary, and how it is drawn is yours: jitter width, size, open or filled, edged or not, n under each group. The numbers never change." |
...
| 6:00-6:45 | Figure preset, preview dialog | tick **Show experimental presets**; **Box + observations (outline)**; **Preview & apply...**; **Apply** | "Experimental publication presets are previewed on your own data: before and after, the list of settings that change, and a check that no data, role, test or threshold changes. Apply - and keep adjusting; a preset is a starting point." |
